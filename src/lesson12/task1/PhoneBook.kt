 ...
 */
class PhoneBook {
    private val phoneList = mutableMapOf<String, MutableList<String>>()
    /**
     * Добавить человека.
    ...
    fun addHuman(name: String): Boolean {
        if (name in phoneList.keys) return false
        else {
            phoneList[name] = mutableListOf()
            return true
        }
    }
    ...
    fun addPhone(name: String, phone: String): Boolean {
        for (elem in phoneList.values) {
            if (elem.contains(phone)) {
                return false
            }
        }
        if (name !in phoneList.keys) return false
        else phoneList[name]?.add(phone)
        return true
    }
    ...
    fun removePhone(name: String, phone: String): Boolean {
        if (name !in phoneList.keys || !phoneList[name]?.contains(phone)!!) return false
        else {
    ...
    fun phones(name: String): Set<String> {
        if (name !in phoneList.keys) return setOf()
        else return phoneList[name]?.toSet() ?: setOf()
    }
    ...
    fun humanByPhone(phone: String): String? {
        for (elem in phoneList.keys) {
            if (phoneList[elem]?.contains(phone)!!) return elem
        }
        return null
    }
    ...
    override fun equals(other: Any?): Boolean {
        if (other is PhoneBook && phoneList.keys == other.phoneList.keys) {
            for (elem in phoneList.keys) {
                if (phoneList[elem]?.sorted() != other.phoneList[elem]?.sorted()) {
                    return false
                }
            }
            return true
